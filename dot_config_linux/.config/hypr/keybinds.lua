...
hl.bind(
	"PRINT",
	hl.dsp.exec_cmd(
		[[grim -o $(hyprctl monitors | grep -B 10 'focused: yes' | grep 'Monitor' | awk '{ print $2 }') -t jpeg ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%m-%s).jpg]]
	)
)
...
hl.bind(
	"CTRL + SHIFT + PRINT",
	hl.dsp.exec_cmd([[grim -t jpeg -g "$(slurp)" ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%m-%s).jpg]])
)
